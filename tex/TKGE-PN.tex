...
（4）基于Transformer的嵌入模型，这类方法的主要思想是利用Transformer模型强大的表达能力来进行知识图谱补全任务，大致可以分为两类，一类主要利用预训练语言模型来学习知识图谱中自然语言文本中的语义，主要包括KG-BERT\upcite{KG-BERT}。另一类则是基于图谱本身的三元组信息来进行链路预测，包括HittER\upcite{HittER}，StAR\upcite{StAR}和Relphormer\upcite{Relphormer}，以及本文先前提出的NALTP模型。这类方法的缺点是模型复杂度较高，模型训练时间长，训练和推理的资源消耗较大。
