...
而在训练和推理时间方面，由于模型复杂度较高，基于Transformer的知识图谱嵌入方法训练和推理所消耗的时间普遍高于传统的知识图谱嵌入方法以及基于图神经网络的嵌入方法。以TKGE-PN模型和基于神经网络的方法KBGAT模型为例，在采用相同的训练环境以及相同的批处理数量的情况下，在WN18RR数据集上TKGE-PN模型训练一个迭代所消耗的时间大约是KBGAT模型的2-3倍左右。
